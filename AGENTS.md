...
Dispatching: `orz_load(format, data, len)` → `audio_engine.c`'s `register_all()` maps format extensions to decoder structs. Only decoders with non-NULL function pointers get registered (weak stubs in `stub_decoders.c` handle excluded decoders).
...
Package.swift uses `cSettings.unsafeFlags` with absolute `-L` paths for the linker (SPM doesn't resolve relative linker paths from the package root).
...
| `Sources/OrzAudioKitCXX/include/audio_engine.h` | Decoder struct & unified API |
| `Sources/OrzAudioKitCXX/dispatch/audio_engine.c` | `register_all()` — format→decoder mapping |
| `Sources/OrzAudioKitCXX/dispatch/orz_dispatch.c` | `orz_load/render/destroy` dispatch |
| `Sources/OrzAudioKit/CDecoderBridge.swift` | Swift-C bridge for native decoding |
...
- Playback via `player.js`: loads orz_audio.wasm, calls `orz_load()` / `orz_render()`, feeds AudioContext
